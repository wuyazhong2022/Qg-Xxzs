...
//var tiaozhan = TTXS_PRO_CONFIG.get("tiaozhan", false);
var ocr_choice = TTXS_PRO_CONFIG.get("ocr_choice", 2);
...
//var shuangren = TTXS_PRO_CONFIG.get("shuangren", false);
...
var yl_on = TTXS_PRO_CONFIG.get("yl_on", false);
var yinliang = TTXS_PRO_CONFIG.get("yinliang", "0");
var zhanghao = TTXS_PRO_CONFIG.get("zhanghao", "");
var comment = TTXS_PRO_CONFIG.get("comment", "为人民服务|共同发展，加油|相互尊重、和平发展|加油厉害我的国|一切为民，实干就对了|努力奋进，服务为民");
...
for (var i = 0; i < list.length; i++) {
    for (var j = i + 1; j < list.length; j++) {
        if (Math.abs(list[i]['bounds'] - list[j]['bounds']) >= eps) {
            if (list[i]['bounds'] > list[j]['bounds']) {
                var tmp = list[i];
                list[i] = list[j];
                list[j] = tmp;
            }
...
for (var i = 0; i < list.length; i++) {
    for (var j = i + 1; j < list.length; j++) {
        if (Math.abs(list[i]['bounds'] - list[j]['bounds']) < eps && Math.abs(list[i]['bounds']['left'] - list[j]['bounds']['left']) >= eps) {
            if (list[i]['bounds']['left'] > list[j]['bounds']['left']) {
                var tmp = list[i];
                list[i] = list[j];
                list[j] = tmp;
            }
...
if (storage.get(engine_version, true)) {
  ...
  let gengxin_rows = "脚本有风险，仅供学习交流;更新内容：;1.增加了防封模块不出滑块验证;2视频文章评论答题都已修复。;3.OCR默认为第三方插件需要下载安装才能答题;4.脚本测试环境：强国V2.5.0 右上3点可以下载;（点击取消不再提示）".split(";");
...
// console.show();
fInfo("学习助手pro" + newest_version + "初始化");
// 初始化宽高
...
if (ocr_choice == 2) {
  ...
  try {
    ...
    alert("未安装第三方OCR插件，点击确认跳转浏览器下载，密码为8888，打开提示手机不能下载，浏览器设置里面切换访问电脑版");
    app.openUrl("https://wwd.lanzout.com/iePtX1zn6owh");
    exit();
  }
}
// sleep(2000);
...
threads.start(function () {
  ...
  //sleep(500);
  toastLog("开始自动获取截图权限");
  var btn = textMatches(/允许|立即开始|START NOW/).findOne(5000);
 //log(btn)
  if (btn) {
    sleep(1000);
    // btn.click();
...
// 下载题库
fInfo("检测题库更新");
const update_info = get_tiku_by_http("https://gitcode.net/m0_67945136/songge_tiku/-/raw/master/info.json");
...
// 寻找 androidx.recyclerview.widget.RecyclerView 或 android.widget.ListView
let recyclerView = frame_box.findOne(className("androidx.recyclerview.widget.RecyclerView"));
let listView = frame_box.findOne(className("android.widget.ListView"));

// 添加日志输出
if (recyclerView) {
//   log("找到 RecyclerView: " + recyclerView);
} else {
//   log("未找到 RecyclerView");
}

if (listView) {
//   log("找到 ListView: " + listView);
} else {
//   log("未找到 ListView");
}

// 点击 RecyclerView 或 ListView 的子项
if (recyclerView) {
  recyclerView.child(1).child(1).child(0).click() || log("尝试再次点击" + recyclerView.child(1).child(1).child(0).child(0).click());
} else if (listView) {
  listView.child(1).child(1).child(0).click() || log("尝试再次点击" + listView.child(1).child(1).child(0).child(0).click());
}
...
function do_quweidati() {
    ...
    // fInfo("等待执行对应答题");
    sleep(2000);// 进入积分项目
    var challengeBtn = text("挑战答题").findOne(2000);  // 查找"挑战答题"控件
    if (challengeBtn != null) {  // 如果找到了"挑战答题"控件
        fInfo("开始挑战赛");
        do_tiaozhan();  // 执行挑战操作
    } else {
        do_duizhan1(4);  // 没有找到"挑战答题"控件，执行对战操作
    }
}
// 封装函数，用于点击RadioButton并检查是否出现"继续挑战"控件
function clickRandomRadioButtonAndCheckContinue() {
    // 等待className("android.widget.RadioButton")控件出现
    ...
    while (true) {
        // 点击随机的 RadioButton
        sleep(1500)
        let radioButtons = className("android.widget.RadioButton").find();
    // console.log("页面中共有 " + radioButtons.length + " 个 RadioButton");
    if (radioButtons.length > 0) {
        let randomIndex = random(0, radioButtons.length - 1);
        let randomRadioButton = radioButtons[randomIndex];
        let bounds = randomRadioButton.bounds();
        let randomX = random(bounds.left, bounds.right);
        let randomY = random(bounds.top, bounds.bottom);
        randomRadioButton.click();
        //console.log("点击了 RadioButton 第 " + (randomIndex + 1) + " 个，坐标位置为：(" + randomX + ", " + randomY + ")");
ran_sleep1();
    }

        // 检查是否出现"继续挑战"控件，如果是，跳出循环
        if (text("继续挑战").exists()) {
            // toastLog("答题结束")
            break;
        }
...
// 封装函数，用于点击RadioButton并检查是否出现"继续挑战"控件
function clickRandomRadioButtonAndCheckContinue1() {
    // 等待className("android.widget.RadioButton")控件出现
    while (!className("android.widget.RadioButton").exists()) {
        sleep(1500); // 等待1秒
        break;
...
function do_duizhan1(renshu) {
 ...
 //entry_jifen_project("趣味答题");
  ...
  // 调用函数
//clickRandomRadioButtonAndCheckContinue();  
        ...
        if (text("继续挑战").exists()) {
          ...
          //if (renshu == 2) {
            ...
            //if (fast_mode) {
              //winReshow();
            //}
          var exit_click = text("退出").findOne(2000);
 if (exit_click != null) {  // 如果找到了"挑战答题"控件
fInfo("点击退出:" + exit_click.click());
} 
          //}
          sleep(1000);
          text("登录").waitFor();
...
fInfo("点击退出:" + exit_click.click());
   ...
   // }
    ...
    if (ocr_choice == 0) {
      // 排序顺序
      console.log("开始执行OCR检测");

let x_results = ocr.detect(img);

//console.log("选项识别：", x_results);
        ...
        if (ans && reg.test(ans) && ans.length == 1) {
            ...
            if (depth(26).text(ans).exists()) {
                ...
                if (xuanxiang) {
                    ...
                    // 随机点击一个选项
                    // log("到这了10")
                   // let randomOption = className("android.widget.RadioButton").findOnce(random(0, 5)).parent();
                   // randomOption.click();
                    //   随机点击一个选项
                     clickRandomRadioButtonAndCheckContinue1()
                }
            }
        }
    }
...
function ocr_rslt_to_txt(result) {
  let txt_list = [];
  let txt = "";
  let top = result[0].bounds.top;

  for (let idx in result) {
    let element = result[idx];
    if(element.bounds){
      if (element.bounds.top >= top - 10 && element.bounds.top <= top + 10) {
        // 如果在同一行
        txt += element.text;
      } else {
        // 如果不在同一行
        txt_list.push(txt.trim()); // 将上一行的文字加入列表
        txt = element.text; // 开始新的一行
        top = element.bounds.top; // 更新当前行的top值
      }
    }
  }
  
  // 将最后一行文字加入列表
  txt_list.push(txt.trim());
...
//这样在处理每个元素之前先检查是否存在边界信息，可以避免出现"Cannot read property 'top' from undefined"错误。希望这次修改能够解决问题。


// 把ocr结果转换为正序的字符串
function ocr_rslt_to_txt1(result) {
  let top = 0;
  let previous_left = 0;
  let txt = "";
  let txt_list = [];
...
// 模拟随机时间0.5-3秒，后期可以用户自定义
function ran_sleep1() {
  return sleep(random(2000, delay_time));
}
// 比较两个字符串相似度
...
function init_wh() {
  ...
  if (device.width == device_h && device.height == device_w) {
    ...
    fError("识别不出设备宽高，建议重启学习助手pro后重新运行脚本");
    sleep(10000);
  }
...
function ocr_test() {
  ...
  try {
    ...
    if (test_time > test_limit) {
      ...
      fError("如偶然变慢，可能为无障碍服务抽风，建议重启学习助手pro后重试");
      sleep(3000);
      return false
    } else {
      fInfo("OCR功能正常");
...
function refind_jifen() {
  ...
  26 == a.depth() ? (jifen_flag = "old", fInfo("检测为旧版界面")) : 23 == a.depth() && (jifen_flag = 0 < a.child(0).child(3).childCount() ? "new1" : "new2", fInfo("检测为新版界面"));
  return a
}
  ...
  var w = floaty.rawWindow(
    <card cardCornerRadius='8dp' alpha="0.8">
      <vertical>
        ...
        <text id='version' textColor="#FFFFFF" textSize="10dip">学习助手+</text>
...
function fInfo(str) {
  ui.run(function () {
    let textView = ui.inflate(<text id="info" maxLines="2" textColor="#7CFC00" textSize="8dip" padding='5 0'></text>, w.container);
...
function fError(str) {
  ui.run(function () {
    let textView = ui.inflate(<text id="error" maxLines="2" textColor="#FF0000" textSize="8dip" padding='5 0'></text>, w.container);
    textView.setText(str.toString());
    w.container.addView(textView);
  });
  console.error(str);
...
function fTips(str) {
  ui.run(function () {
    let textView = ui.inflate(<text id="tips" maxLines="2" textColor="#FFFF00" textSize="5dip" padding='5 0'></text>, w.container);
    textView.setText(str.toString());
    w.container.addView(textView);
  });
  console.info(str);
...
function xxqg(userinfo) {
  ...
  if (ocr_test()) {
    if (true == quweidati && ("old" == jifen_flag && 3 >= parseInt(jifen_list.child(jifen_map["趣味答题"]).child(2).text().match(/\d+/)[0]) || "new1" == jifen_flag && 3 >= parseInt(jifen_list.child(jifen_map["趣味答题"]).child(3).child(0).text()) || "new2" == jifen_flag && 3 >= parseInt(jifen_list.child(jifen_map["趣味答题"]).child(3).text().match(/\d+/)[0]))) {
 ...
 var found = false;
while (!found) {
  var b = text("我的").findOne(1000); // 查找"我的"，超时时间设置为1秒
  if (b) {
    // log("找到\"我的\"，点击");
    b.click(); // 找到"我的"，点击
    found = true; // 设置found为true，结束循环
  } else {
    // log("未找到\"我的\"，继续查找");
    back();
    sleep(2000);
...
//小哥卡密验证////小哥卡密验证////小哥卡密验证////小哥卡密验证////小哥卡密验证//
