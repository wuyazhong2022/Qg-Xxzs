...
function refind_jifen() {
  ...
  21 == a.depth() ? (jifen_flag = "old", fInfo("检测为旧版界面")) : 24 == a.depth() && (jifen_flag = 0 < a.child(2).child(3).childCount() ? "new1" : "new2", fInfo("检测为新版界面"));
  return a
}
...
function xxqg(userinfo) {
  ...
  true == pinglun && ("old" == jifen_flag && "0" == jifen_list.child(jifen_map["评论"]).child(2).text().match(/\d+/)[0] || "new1" == jifen_flag && "0" == jifen_list.child(jifen_map["评论"]).child(2).child(0).text() || "new2" == jifen_flag && "0" == jifen_list.child(jifen_map["评论"]).child(3).text().match(/\d+/)[0]) && (toastLog("开始评论"), do_pinglun(), jifen_list = refind_jifen());
